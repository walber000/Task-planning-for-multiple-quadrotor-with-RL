...
class client(Thread):
	...
	def run(self):
		try:
			...
			buff_size = 1024
			while(1):
				global t0, t1, posX_cm, posY_cm, posZ_cm, ang, send_data
				# Client
				# ip = '127.0.0.1'
				if(send_data):
					client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
					client_socket.settimeout(1.0)
					data = [round((t1-t0),3), round(posX_cm,2), round(posY_cm,2), round(posZ_cm,2), round(ang,1)]
					msg = struct.pack('<5f', *data)
...
def detect_cam1_obj(img1, h):
	...
	posX = cx
	posY = cy
	ymin = 41 #38 antig, 41 07/08/2020
	ymax = 175 #174 antigo,175 07/08/2020
	xlim_min = 0 + 92*h/hmax
	xlim_max = 164 - 72*h/hmax
	ylim_min = ymin + (90-ymin)*h/hmax
	ylim_max = ymax - (ymax-90)*h/hmax
...
def detect_cam2_obj(img2, h):
	...
	#posicao corrigida devido a altura
	posX = cx; posY = cy
	ymin = 104 #107 07/08/2020, usando antigo
	ymax = 279-32 #(279-32) antig, (279-29) 07/08/2020
	xlim_min = 0 + 92*h/hmax
	xlim_max = 164 - 72*h/hmax
	ylim_min = ymin + (226-ymin)*h/hmax
	ylim_max = ymax - (ymax-226)*h/hmax
