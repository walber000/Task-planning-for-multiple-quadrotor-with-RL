...
if(success):
	try:
		...
		tol = 5 #tolerancia de waypoint (quadrada no momento para controle45, nao circular)
		tol_yaw = 4.6 #tol_yaw eh tolerancia para correcao do angulo
		tol_ang = 10 #tol_ang eh a tolerancia de erro de ang para acontecer correct_yaw
		loaded = 0
			...
			for j in range(size2): #waypoints em cada trajetoria
				if(i==0 and j==0):
					...
					if(not(x==0 and y==0)):
						...
						ang_partida = math.atan2(dy,-dx)*180/math.pi #usando -dx para corrigir diferenca entre angulo calculado e angulo real/camera
