%{
        ...
        rota entre nÛ inicial e final dentro de uma grade

        Respons·vel por ROMEO.cpp: Cairo L. Nascimento Jr.
        Autor de RomeoPlus.m: Walber Lima P. Junior
        27/08/19
%}

function [gsuc, count, linha, waypoints] = Astar_old(start, goal, desenhar, cor)
%% VARIAVEIS
i_inic = start(1);
j_inic = start(2);
i_final = goal(1);
...
for i = 1:num_lin
    for j = 1:num_col
        % if map(i,j) == 0 -> espaÁo vazio
        % if map(i,j) == 1 -> obstaculo
        if map(i,j) == 2  %NÛ inicial do mapa global vira espaÁo vazio
            map(i,j) = 0;
        end
        if map(i,j) == 3  %NÛ final do mapa global vira espaÁo vazio
            map(i,j) = 0;
        end
        if map(i,j) == 4  %Ponto de entrega do mapa global vira espaÁo vazio
            map(i,j) = 0;
        end
    end
end
...
%% MATRIZ H (custo de cada nÛ para nÛ final)
...
%% PLANEJAR TRAJET”RIA (A*)

OPEN(1,1) = num_col*(i_inic-1)+j_inic; %nÛ inicial vai para OPEN
OPEN(1,2) = -1; %antecessor do nÛ inicial = -1
...
while 1
    ...
    N=OPEN(1,1);
    gn=GOPEN(1); % Toma o seu custo gn do nÛ N
...
in=fix(N/num_col)+1;  % in,jn = posiÁ„o do nÛ N na grade */
jn=mod(N,num_col);
if jn==0
    jn = num_col; in = in-1;
end

Np=CLOSED(nclosed,2);  % recupera o pai do nÛ N = Np*/
if (in>=1) && (jn>=1) && (in<=num_lin) && (jn<=num_col)
    if map(in,jn)==2
        ip=-1; jp=-1; % ip,jp = posiÁ„o do nÛ Np na grade */
    else
        ip=fix(Np/num_col)+1;
        jp=mod(Np,num_col);
        if jp==0
    ...
    if (map(in,jn)==2)
        tetac=teta0; % se for nÛ de partida */
    else
...
for i=in-1:in+1
    for j=jn-1:jn+1
        ...
        if ( (i>=1) && (j>=1) && (i<=num_lin) && (j<=num_col) && (map(i,j)~=1) && (suc~=N) && (CLOSED(nclosed,2)~=suc) && (fim_da_busca==0))
            ...
            %Calcular o Éngulo de partida tetap considerando o
            ...
            %se o n¢ sucessor for o n¢ final ent∆o indicar com o
            %flag fim_da_busca=1 e armazenar o valor de gn como sendo
            %o custo total da rota
            if ((i==i_final) && (j==j_final))
                fim_da_busca=1;       % O n¢ final foi atingido */
                %custo_rota=gsuc;      % custo total da rota      */
            end
            
            novo=1;
            
            %disp("checando se sucessor ja est· em OPEN...")
            for a=1:nopen
                %disp("suc: "+suc+", OPEN(a,1): "+OPEN(a,1))
                if suc==OPEN(a,1)
                    %disp("sucessor ("+i+","+j+") j· existe em OPEN")
                    novo=0;
                    if (gsuc<GOPEN(a))
                        %disp("mantendo o SUC")
                        GOPEN(a)=gsuc;
            ...
            for a=1:nclosed
                if (suc==CLOSED(a,1))
                    %disp("sucessor ("+i+","+j+") j· existe em CLOSED")
                    novo=0;
                    if (gsuc<GCLOSED(a))
                        %disp("mantendo o SUC")
                        GCLOSED(a)=gsuc;
