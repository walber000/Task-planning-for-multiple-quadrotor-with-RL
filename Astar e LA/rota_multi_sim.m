...
function [dist, waypoints2] = rota_multi(map,prob1, prob2, p_carga, obj_qtd, demanda_zona)
...
w=1;
aux_w = zeros(1,num_transp);
waypoints_prev = zeros(num_transp,2);
%% Movimentação
        ...
        if(finished(n)<2)
            ...
            if(loading(n)~=0)
                ...
                if((pos1(n)==pos2(n)))
                    ...
                    [teta(n), gsuc(n), count(n), linha(n), waypoints(n,:)] = Astar_multi_sim_LA(map,[i1(n) j1(n)], [i2(n) j2(n)], teta(n), desenhar, cor(n), obs);       
                    delete(speed(n));
                    if(gsuc(n)==0)
                        disp(n+" parado")
                        speed(n) = annotation('textbox', [dx+ddx*n, dy+ddy*n, barx, 0.03], 'string', "vel"+n+":");
...
disp("end1")
%% incluir valor 3 no terceiro indice do waypoint final
waypoints2_backup = waypoints2;
for i=1:size(waypoints2, 1)
    for j=1:size(waypoints2, 2)
        if(waypoints2{i,j} == [i_inic(i) j_inic(i) 0])
            waypoints2{i,j} = [i_inic(i) j_inic(i) 3];
            break
        end
    end
end         

%save('waypoints2.mat','waypoints2')
...
function pos = decidir_carga(n)
    global k2 mov1 obj finished no_inic debug mov_carga num_obj_decididos num_obj2
    %global obj_qtd_temp carga_iter
    if(debug) disp("transp "+n+", k2= "+k2(n)); end
    mov1(n,k2(n))=mov_carga(n,k2(n));
    if(mov1(n,k2(n))==0 || num_obj_decididos>num_obj2)
