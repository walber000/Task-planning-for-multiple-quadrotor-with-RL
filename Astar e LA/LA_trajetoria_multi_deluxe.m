...
NIterMax=20000; % no. de iterações total de aprendizado
        ...
        for n = 1:num_transp
            if(finished(n)<2)
                ...
                if(pos1(n)==pos2(n))
                    ...
                    gsuc(n) = 0; count(n) = 0; waypoints{n}(1,:) = [i1(n) j1(n)];
                else
...
%                     disp(n+" "+pos1(n)+" "+pos2(n))
                    %log de A*
                    log_found = 0;
                    if(count_wp(n)==size(waypoints{n},1))
                        waypoints{n} = [];
                        count_wp(n) = 1;
                    end
                    if(size(waypoints{n},1))
                        [teta(n), gsuc(n)] = teta_e_custo(teta(n), waypoints{n}(count_wp(n):count_wp(n)+1,:));
                        count_wp(n) = count_wp(n) + 1;
                    else
                        if(use_log)
                        ...
                        if(log_found==0)
                            aux = teta(n);
                            [teta(n), gsuc(n), count(n), ~, waypoints{n}(:,:)] = Astar_multi_sim(map,[i1(n) j1(n)], [i2(n) j2(n)], teta(n), desenhar, cor(n), obs);       
                            A_count = A_count+1;
                            if(use_log)
                                log(end+1,:)=[i1(n) j1(n) i2(n) j2(n) aux teta(n) gsuc(n)];
                                log_wp{end+1}= waypoints{n};
                            end
                        end
                    end
                end
                ...
                pos1(n) = (waypoints{n}(count_wp(n),1)-1)*num_col + waypoints{n}(count_wp(n),2);
                [i1(n),j1(n)]=pos_to_ij(pos1(n));
...
%         aut1min(n) = min(max(prob1(find(max(prob1(:,:,n),[],2)>0.6),:,n),[],2));
%         aut2min(n) = min(max(prob2(find(max(prob2(:,:,n),[],2)>0.6),:,n),[],2));
