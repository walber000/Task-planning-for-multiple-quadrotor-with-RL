...
rand('state',0);
desenhar = 0; pausar = 0; tempo_pause = 0; debug = 1; debug_mov = 1; use_log=1;
eug_is_in = 0;
if(desenhar) plotMap_sim(map); end
t1=now;

%% Definição do mapa
% filename = 'walber_2.jpg';
% map2 = gazimg2grid(filename);
...
if(eug_is_in)
    ...
    %map = gazimg2grid('walber_2.jpg');
    %map = cast(map,'uint8');
    % map(1:end+2,end:end+2)=1;
    % map(end-2:end,1:end)=1;

    load_names = {'walber_load_1','walber_load_2',',walber_load_3'};
    unload_names = {'walber_unload_1','walber_unload_2'};
    model_states = rossubscriber('/gazebo/model_states');
    msg = receive(model_states, 5);
    str = msg.Name;

    % Get load positions
    load_pos = zeros(length(i),3);
    for j=1:length(i)
        idx = strfind(str, load_names{j});
        i = find(~cellfun(@isempty, idx));
        load_pos(j, :) = [msg.Pose(i).Position.X; msg.Pose(i).Position.Y; msg.Pose(i).Position.Z];
        grid_load_pos(1:2, j) = load_pos(j, 1:2)' + 10; %offset, pois (0,0) estava no centro do mapa
        grid_load_pos(1, j) = round(grid_load_pos(1, j)*num_col/size_mi);
        grid_load_pos(2, j) = round(grid_load_pos(2, j)*num_lin/size_mj);
        map(grid_load_pos(2, j),grid_load_pos(1, j)) = 3; %define no mapa como carga
    end

    % Get unload positions
    unload_pos = zeros(length(i),3);
    for j=1:length(i)
        idx = strfind(str, unload_names{j});
        i = find(~cellfun(@isempty, idx));
        unload_pos(j, :) = [msg.Pose(i).Position.X, msg.Pose(i).Position.Y, msg.Pose(i).Position.Z];
        grid_unload_pos(1:2, j) = unload_pos(j, 1:2)' + 10; %offset, pois (0,0) estava no centro do mapa
        grid_unload_pos(1, j) = round(grid_unload_pos(1, j)*num_lin/20);
        grid_unload_pos(2, j) = round(grid_unload_pos(2, j)*num_col/20);
        map(grid_unload_pos(2, j),grid_unload_pos(1, j)) = 4; %define no mapa como carga
    end
    % Set drones' inicial positions (i j)
    i_base = num_col/2; j_base = num_lin/2;
    n = 3;
    for i=1:n
        inicial_pos(i,:) = [i_base+i*num_col/size_mi j_base 0]; %in sim coord, meters
        map(inicial_pos(i,2),inicial_pos(i,1)) = 2;
    end
else 
    load_pos = [-8 8;8 8;0 -5];
    unload_pos = [-8 -8;8 -8];
    inicial_pos = [1 0; 2 0; 3 0];
    
    load_pos = [0 0;0 0;0 0;0 0];
    unload_pos = [0 8.5 0; -8.5 0 0; 8.5 0 0; 0 -8.5 0];
    inicial_pos = [0 8 0; -8 0 0; 8 0 0; 0 -8 0];
    
    load_pos = [-8 8;8 8;4 -8;8 0];
    unload_pos = [-8 -8 0; 8 -8 0; 0 8 0];
    inicial_pos = [0 0 0; 2 4 0; 4 0 0];
    
    for j=1:length(load_pos)
        grid_load_pos(j,1) = load_pos(j,1) + 10; %offset, pois (0,0) estava no centro do mapa
        grid_load_pos(j,2) = 10 - load_pos(j,2); %offset
        grid_load_pos(j,1) = round(grid_load_pos(j,1)*num_lin/20);
        grid_load_pos(j,2) = round(grid_load_pos(j,2)*num_col/20);
        map(grid_load_pos(j,2),grid_load_pos(j,1)) = 3; %define no mapa como zona carga
    end
    for j=1:length(unload_pos)
        grid_unload_pos(j,1) = unload_pos(j,1) + 10; %offset, pois (0,0) estava no centro do mapa
        grid_unload_pos(j,2) = 10 - unload_pos(j,2); %offset
        grid_unload_pos(j,1) = round(grid_unload_pos(j,1)*num_lin/20);
        grid_unload_pos(j,2) = round(grid_unload_pos(j,2)*num_col/20);
        map(grid_unload_pos(j,2),grid_unload_pos(j,1)) = 4; %define no mapa como zona descarga
    end
    for j=1:length(inicial_pos)
        grid_inicial_pos(j,1) = inicial_pos(j,1) + 10; %offset, pois (0,0) estava no centro do mapa
        grid_inicial_pos(j,2) = 10 - inicial_pos(j,2); %offset
        grid_inicial_pos(j,1) = round(grid_inicial_pos(j,1)*num_lin/20);
        grid_inicial_pos(j,2) = round(grid_inicial_pos(j,2)*num_col/20);
        map(grid_inicial_pos(j,2),grid_inicial_pos(j,1)) = 2; %define no mapa como inicial pos
    end
end
...
%                     end
                    ...
                    if(log_found==0)
                        aux = teta(n);
                        %[i1(n) j1(n)], [i2(n) j2(n)]
                        [teta(n), gsuc(n), count(n), ~, waypoints] = Astar_multi_sim_LA(map,[i1(n) j1(n)], [i2(n) j2(n)], teta(n), desenhar, cor(n), obs);       
                        A_count = A_count+1;
                        if(use_log)
...
% save('map2.mat','map')
[dist,waypoints2]=rota_multi_sim(map,prob1,prob2,p_carga, obj_qtd,demanda_zona);
