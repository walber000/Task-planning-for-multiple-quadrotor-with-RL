...
function [tetap, gsuc, count, linha, waypoints] = Astar_multi_sim(map,start, goal, teta0, desenhar, cor, obs, raio)
%% VARIAVEIS
i_inic = start(1);
j_inic = start(2);
i_final = goal(1);
...
gsuc = 0;
linha = line(0,0);
%raio = 6; %raio de celulas ao redor do drone consideradas como obstaculos
...
good_to_go = 0;
% checar se pos inicial nao foi obstruida
for i = 1:num_lin
    for j = 1:num_col
        if map(i,j) == 2  %Nó inicial do mapa global vira espaço vazio
            good_to_go = 1;
