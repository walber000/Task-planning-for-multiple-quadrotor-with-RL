...
def correct_yaw(base,tol):
	global ang, posX_cm
	count = 0
	vel_base = 15*0.66 #66% for drone2
	vel = vel_base
	slow_vel_ang = 18
	high_vel_ang = 36
...
def controleDiag(x_d,y_d,tol):
	...
	while (dist>tol or (posX_cm > 900)):
		if(posX_cm < 900):
			...
			" pitch: ", pitch, " roll: %.2f" % (roll_base+roll_b), " vel_x: %.3f" %mambo.sensors.speed_x, " vel_y: %.3f" %mambo.sensors.speed_y)
			#mambo.smart_sleep(0.1)
		else:
			print("camera missed")
			mambo.fly_direct(roll=0, pitch=0, yaw=0, vertical_movement=0, duration=0.2)
...
def altitude(h,vel):
	...
	count = 0
	tol=2.5
	vel = vel*(1+0.67*loaded)*0.66 #66% for drone2
	#tol = 7
...
def callback(data):
	global timestamp, posX_cm, posY_cm, posZ_cm, ang, vel_record
	global X, Y, Z, A, VX, VY, VZ, E
	timestamp = data.data[0]; posX_cm = data.data[5]; posY_cm = data.data[6]; posZ_cm = data.data[7]; ang = data.data[8]
	if(savefile):
		X.append(posX_cm)
		Y.append(posY_cm)
		Z.append(posZ_cm)
...
def drone_node():
	rospy.init_node('drone2_node', anonymous=True, disable_signals=True)
...
if(success):
	try:
		...
		# mambo.fly_direct(roll=-12, pitch=10, yaw=0, vertical_movement=0, duration=2.5)
		# mambo.smart_sleep(2)
		# mambo.safe_land(5)
		# mambo.disconnect()
	...
	if(savefile):
		print("saving...")
		# print("%.1f"%range(len(Z)))
		# print("%.1f"%range(len(VX))
