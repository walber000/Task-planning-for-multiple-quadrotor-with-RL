...
class server(Thread):
	...
	# def run(self):
		try:
			...
			# server_socket.setsockopt(socket.SOL_IP, socket.IP_MULTICAST_IF, socket.inet_aton(host))
			# server_socket.setsockopt(socket.SOL_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(ip) + socket.inet_aton(host))		
			...
			# while True:
				...
				# server_socket.recv(10240))
				# data = struct.unpack('<9f', data)
				# [_,aux, _, _, _, _, _, _, _] = data
				# if(aux>0):
					# [timestamp, posX_cm, posY_cm, posZ_cm, ang, _, _, _, _] = data
				# if(self.killed):
					# break
...
def callback(data):
	...
	timestamp = data.data[0]; posX_cm = data.data[1]; posY_cm = data.data[2]; posZ_cm = data.data[3]; ang = data.data[4]
	if(savefile):
		X.append(posX_cm)
		Y.append(posY_cm)
		Z.append(posZ_cm)
