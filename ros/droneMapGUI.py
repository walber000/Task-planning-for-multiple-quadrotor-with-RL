...
class DroneGUI:
    ...
    def draw_room(self, length, height):
        ...
        filemenu.add_separator()
        filemenu.add_command(label="Start!", command=self.start_movement)
    ...
    def start_movement(self):
        ...
        goal = [-1,-1]
        pos = [0,0]
        for i in range(self.room_map.shape[1]):
            for j in range(self.room_map.shape[0]):
                if (self.room_map[i, j] == self.start_id):
                    start = [i,j]
					
        for i in range(self.room_map.shape[1]):
            for j in range(self.room_map.shape[0]):
                if (self.room_map[i, j] == self.goal_id):
                    goal = [i,j]
        ...
        # while(distance_x!=0):
		    # # clear whatever was there
            # self.clear_obstacle(pos[0], pos[1])
            # self.room_map[map_x, map_y] = 0

            # # and save the movement into the map
            # self.room_map[map_x, map_y] = self.drone_id
            # self.draw_obstacle(pos[0],pos[1], self.factor, color=self.drone_color, map_x=map_x, map_y=map_y)
        
    def position(self, x, y):

        factor = 10 * self.scale_val
        lower_x = int(x / factor) * factor
        lower_y = int(y / factor) * factor

        map_x = int(x / self.factor)
        map_y = self.room_map.shape[1] - int(y / self.factor) - 1
        #map_y = int(y / self.factor)
		
		# if(self.room_map.shape[0]<=map_x):
			# map_x = self.room_map.shape[0] - 1
        if(map_x==self.old_x and map_y==self.old_y):
            pass
        else:
            self.draw_obstacle(self.old_lower_x,self.old_lower_y, self.factor, color="#ffffe6", map_x=self.old_x, map_y=self.old_x)
            self.room_map[map_x, map_y] = self.drone_id
            self.draw_obstacle(lower_x,lower_y, self.factor, color=self.drone_color, map_x=map_x, map_y=map_y)
            self.old_x = map_x; self.old_y = map_y
            self.old_lower_x = lower_x; self.old_lower_y = lower_y
